...
- Systematic NP groups (`--num-sig-yield-systs M` / alias `--num-systs`, `--num-sig-width-systs`, `--num-bkg-norm-systs`, `--num-bkg-shape-systs`): shared NPs `alpha_syst<j>` (sig-yield, legacy names) or `alpha_<kind>_syst<j>` with constraints `constr_<np-name>` and global observables `nom_<np-name>`. Each group enters a channel via one multiplicative `RooStats::HistFactory::FlexibleInterpVar` response `resp_<kind>_<ch>` (asymmetric up/down variations, interpolation code from `--interp-code`, default 4) folded into `nsig_tot_<ch>`, `sigma_<ch>` (base node `sigma_base_<ch>`), `nbkg_tot_<ch>`, or `tau_eff_<ch>` respectively. The free baselines `nbkg_<ch>`/`tau_<ch>` stay unconstrained.
